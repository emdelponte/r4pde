...
get_era5 <- function(data, days_around, date_col, study_col = "study",
                    pars = c("temperature_2m", "relative_humidity_2m", 
                             "precipitation", "dewpoint_2m"),
                    models = NULL) {

  # Helper function to fetch and aggregate data for a single location
  get_single_era5 <- function(data_row, days_around, date_col, study_col, pars, models) {
    ...
    if (is.null(json_data$hourly)) return(data.frame())
  ...
  results <- purrr::map_dfr(1:nrow(data), function(i) {
    pb$tick()
    get_single_era5(data[i, , drop = FALSE], days_around, date_col, study_col, pars, models)
  })
