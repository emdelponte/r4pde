...
get_brdwgd <- function(data, days_around, date_col, study_col = "study",
  ...
  date_list <- lapply(1:nrow(data), function(i) {
    ...
    if (direction == "back") {
      seq(base_date - days_around, base_date, by = "day")
    } else if (direction == "forth") {
      seq(base_date, base_date + days_around, by = "day")
    } else {
  ...
  final_res <- final_df %>%
    dplyr::select(
      dplyr::any_of(c("date", "study", "latitude", "longitude", lon_col, lat_col)),
      dplyr::everything()
    )
